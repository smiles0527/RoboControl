...
/// Third-degree spherical-radial cubature rule for nonlinear estimation.
/// </summary>
public class CubatureKalmanFilter
{
    ...
    public void Predict(Vector<double> u)
    {
        ...
        var cubaturePoints = GenerateCubaturePoints(_x, _P);
        int numPoints = 2 * _n;
        
        // Propagate through dynamics
        var propagatedPoints = new Vector<double>[numPoints];
        for (int i = 0; i < numPoints; i++)
        {
            propagatedPoints[i] = _f(cubaturePoints[i], u);
        }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            _x += propagatedPoints[i] / numPoints;
        }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            ...
            _P += diff.OuterProduct(diff) / numPoints;
        }
    }
    ...
    public void Update(Vector<double> z)
    {
        ...
        var measuredPoints = new Vector<double>[numPoints];
        for (int i = 0; i < numPoints; i++)
        {
            measuredPoints[i] = _h(cubaturePoints[i]);
        }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            zPred += measuredPoints[i] / numPoints;
        }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            ...
            Pzz += diff.OuterProduct(diff) / numPoints;
        }
        ...
        for (int i = 0; i < numPoints; i++)
        {
            var xDiff = cubaturePoints[i] - _x;
            var zDiff = measuredPoints[i] - zPred;
            Pxz += xDiff.OuterProduct(zDiff) / numPoints;
        }
    ...
    private Vector<double>[] GenerateCubaturePoints(Vector<double> x, Matrix<double> P)
    {
        int numPoints = 2 * _n;
        var points = new Vector<double>[numPoints];
        
        // Square root of covariance
        var sqrtP = (System.Math.Sqrt(_n) * P).Cholesky().Factor;
...
public class EnsembleKalmanFilter
{
        ...
        Matrix<double> measurementNoise)
    {
        ...
        _ensemble = new Vector<double>[ensembleSize];
